...
<style>
    ...
    .pf-role-badge.admin{background:var(--pf-navy);color:#fff;}
    .pf-role-badge.user{background:var(--pf-green);color:#fff;}

    .pf-quicklinks{border-top:1px dashed #EFE6C0;padding-top:16px;text-align:left;}
    .pf-quicklink{display:flex;align-items:center;gap:10px;padding:10px 8px;border-radius:10px;color:var(--pf-navy);text-decoration:none;font-size:.88rem;font-weight:600;}
    .pf-quicklink:hover{background:var(--pf-cream);color:var(--pf-navy);}
    .pf-quicklink i:first-child{color:var(--pf-green);font-size:1rem;}
    .pf-quicklink i:last-child{margin-left:auto;font-size:.8rem;color:#707378;}
...
<div class="container{{ Auth::user()->role !== 'admin' ? ' py-4' : '-fluid' }}">
    ...
    <div class="pf-side">
        ...
        <span class="pf-role-badge {{ $user->role }}">
            {{ ucfirst($user->role) }}
        </span>
    </div>
